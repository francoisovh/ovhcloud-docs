...
Vous pouvez effectuer cette opération via l'API OVHcloud ou depuis l'espace client OVHcloud, dans la nouvelle console.
...
L'appel retourne une tâche.
  ...
  <Tab label="Espace client (nouvelle console)">

La clé d'appairage se saisit uniquement dans la nouvelle console. Ouvrez la <ManagerLink to="/beta/#/network/cloud-connect">nouvelle console de l'espace client OVHcloud</ManagerLink> et sélectionnez votre service.
...
| Suppression de la configuration PoP | La configuration PoP est supprimée lorsque vous dissociez la clé d'appairage (<code className="action">Dissocier la clé</code> dans la nouvelle console) ou lorsque vous résiliez la connexion. La dissociation du vRack retire également la clé. Le service n'est pas résilié et peut être associé de nouveau à Google. |
