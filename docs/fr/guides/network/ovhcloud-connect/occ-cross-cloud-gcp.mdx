...
:::info
Les étapes 1 et 2 sont interchangeables. Vous pouvez également commencer dans la console Google Cloud en créant le rattachement de VLAN Partner Interconnect, en cochant l'option d'activation automatique pour ne pas avoir à y revenir, puis effectuer les étapes restantes dans l'espace client OVHcloud.
:::
...
:::info
OVHcloud n'émet aucune clé de service pour les connexions avec Google Cloud. Le service est activé à l'aide d'une clé d'appairage générée dans votre console Google Cloud (voir l'étape suivante).
:::
...
Transmettez la clé d'appairage à OVHcloud depuis votre espace client OVHcloud ou via l'API OVHcloud, afin que le circuit virtuel vers Google soit déployé. Copiez la clé exactement telle que Google l'a émise, en conservant l'intégralité de la chaîne `{uuid}/{region}/{availabilityDomain}`.

{/* DRAFT: À vérifier — la clé est transmise via l'espace client ou l'API (source : spécification de service). Confirmer l'écran et le champ exacts, et si elle est transmise avec la commande ou après livraison. */}
