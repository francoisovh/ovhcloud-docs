...
Tout d'abord, survolez avec votre curseur le bouton bleu ➕ **NEW TABLE** sur le côté droit de l'écran. Cela affichera les options de création :
...
:::info
Pour les besoins de ce tutoriel, nous allons procéder avec **Create from a Connectors source.** Pour créer une table en téléversant un fichier, reportez-vous à la documentation [create tables](/guides/public-cloud/data-platform/lakehouse-manager-tables#créer-une-table).
:::
...
:::info
Pour le moment, cet attribut n'est pas physiquement spécifié, mais ne vous inquiétez pas ! Cela sera fait bientôt dans un autre composant : le Data Processing Engine.
:::
...
Passez à la page **Attributes**. Cette page répertorie tous les attributs physiques et virtuels de votre modèle de données, ainsi que le lineage de votre Project.
