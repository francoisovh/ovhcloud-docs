...
:::info
Les options *Build the table, load the table once, and generate Load action for later* peuvent être désactivées et chaque étape peut également être réalisée individuellement. Cela peut être exploré en détail [ici]
:::
...
:::info
Notez que la Platform utilise les informations de métadonnées capturées lors de l'étape Analyzer pour créer automatiquement les tables et attribuer les noms et types d'attributs. Les fichiers sources fournis sont prêts à l'emploi ; toutefois, dans un Project réel, vous devriez utiliser l'Analyzer pour vérifier les sources de données avant de les glisser-déposer dans la page Tables.
:::
...
:::info
Pour le moment, cet attribut n'est pas physiquement spécifié, mais ne vous inquiétez pas ! Cela sera fait prochainement dans un autre composant : le Data Processing Engine.
:::

[En savoir plus sur les Tables](/guides/public-cloud/data-platform/lakehouse-manager-tables)
...
Basculez vers la page **Attributes**. Cette page liste tous les attributs physiques et virtuels de votre modèle de données, ainsi que le lineage de votre Project.
...
:::info
Notre Marketplace vous donne accès à une dizaine d'actions organisées pour démarrer rapidement vos Projects de traitement de données : des actions *load*, des actions *aggregate*, des actions *delete*, etc. Si vous ne trouvez pas ce dont vous avez besoin dans le catalogue, vous pouvez toujours recourir à une action *custom* qui vous permet d'**exécuter n'importe quel morceau de code Python 3+** dans le cadre de vos pipelines de données.
:::

[En savoir plus sur les actions personnalisées](/guides/public-cloud/data-platform/dpe-actions-custom).
...
[En savoir plus sur les actions Aggregate](/guides/public-cloud/data-platform/dpe-actions-aggregate)
...
[En savoir plus sur la configuration des préférences d'exécution.](/guides/public-cloud/data-platform/dpe-jobs-preferences)
...
Vous avez maintenant terminé la section Data Engineering du tutoriel *Getting Started* 🌟
